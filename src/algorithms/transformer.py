...
class Transformer:
    ...
    def _get_bart_summary(self):
        ...
        summary = summarization(self.text, min_length=self.min_length, max_length=self.max_length)[0]['summary_text']
        return summary

    def _get_t5_summary(self):
        summarization = pipeline("summarization", model=self.model, tokenizer=self.tokenizer, framework=self.framework)
        summary = summarization(self.text, min_length=self.min_length, max_length=self.max_length)[0]['summary_text']
        return summary
